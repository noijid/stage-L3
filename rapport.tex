...
On suppose dans cette partie, les polynômes $f_1$ et $f_2$ fixés et on note $m$ leur racine commune. On se munit de deux normes multiplicatives $N_1$ et $N_2$ sur les anneaux $\z[\al_1]$ et $\z[\al_2]$.  On se fixe alors deux bases de nombres premiers $FB_1$ et $FB_2$ dans $\z[\al_1]$ et $\z[\al_2]$, et on cherche des paires de nombres premiers entre eux $a_i$ et $b_i$ tel que $a_i - \al_1b_i$ et $a_i - \al_2b_i$ soient friables. Cette étape est très importante dans l'algorithme du crible algébrique car cette recherche de $a_i, b_i$ peut être faite de manière simultanée (en programmation parallèle) et permets donc de gagner du temps de calcul, si le besoins en est. 
On utilise alors un la même procédure que dans le crible quadratique, en utilisant les formes factorisées des nombre précédents pour obtenir une congruence de carrés. Enfin, une fois qu'on a nos 2 carrés, en les injectants dans $\z/n\z$, en remplaçant $\al_1$ et $\al_2$ par $m$ on obtient la congruence souhaitée.


\subsubsection{Choix des polynômes}
Dans le cas général, on cherche deux polynômes, $f_1$ et $f_2$, ayant des coefficients pas trop grands, afin de ne pas avoir des normes trop grandes pour les $a - \al b$ dans $\z[\al_1]$ et $\z[\al_2]$. En effet, plus les normes des nombres seront petites, plus il sera probable qu'ils soient friables, et donc cela permets d'accélérer la recherche de congruences. On choisit donc, en général, un certain entier  $m$, puis on prends pour les coefficients de $f_1$, ceux de l'écriture de $n$ en base $m$, et on prends $f_2(x) = x - m$ de sortes que $f_1$ et $f_2$ et m comme racine commune modulo $n$. On ne veut pas non plus $f_1$ de degré trop grand, donc on ne prends pas m trop petit. Selon \cite{ref6}, on fixe $d = 4$ ou $5$ le degré de $f_1$ et on prends $m$ proche de $n^{\frac{1}{d+1}}$. On obtient de plus $f_1$ unitaire, ce qui facilite le calcul des normes.
