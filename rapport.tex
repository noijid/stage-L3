...
\vspace{1cm}
    ...
    \begin{minipage}{0.4\textwidth}
      \begin{flushright} \large
        \emph{Tuteur :} M. Laurent \textsc{Grémy}\\
      \end{flushright}
    \end{minipage}
...
RSA est le système de cryptographie le plus utilisé aujourd'hui. C'est un système de chiffrement asymétrique qui repose sur la difficulté de factorisé un grand nombre (la clé publique produit de 2 nombres premiers (qui constituent la clé privée). Ainsi, la factorisation d'entier, est un problème très compliqué aujourd'hui, trouver un algorithme efficace pour résoudre ce problème renverserai les systèmes de cryptographie.

\vspace{0.5cm}

La complexité du problème de la factorisation d'un grand nombre est encore ouvert aujourd'hui. On sait que ce problème est à la fois dans NP et dans co-NP, mais sa NP-complétude n'a pas été montré. Toutefois, le test de primalité AKS est un algorithme permettant de savoir si un nombre $N$ est premier ou non en temps polynomial. Ainsi, l'intérêt des algorithmes de cribles étudiés en complexité sous-exponentielle (qui sera donnée de manière plus précise plus loin) est certain.
...
\subsubsection{Le crible linéaire}
...
Cette étape n'est pas obligatoire, et peut être réalisé en même temps que le crible. Toutefois, ce n'est pas le cas dans l'implémentation de Small-NFS, qui est celle dont je suis parti. Donc elle existe dans mon algorithme.
