...
Mon stage au sein de cette équipe a consisté essentiellement à l'étude, la compréhension et l'écriture du code d'algorithme de factorisation de grands entiers : Quadratic Sieve (crible quadratique) et Number Field Sieve (crible algébrique). Puis, à  modifier le code du crible algébrique afin d'essayer de l'accélérer. Ainsi, en comparant des temps de calculs, à défaut d'avoir des machines aussi puissantes et des codes aussi efficaces que ceux qui réalisent les records de factorisation, cela permets de savoir si ces records sont améliorables, et dans quelle mesure. Ces deux algorithmes ont pour but de factoriser un entier en cherchant des congruences de carrés. Ils ont un intérêt certains puisque QS a notamment permis de factoriser RSA-129, le 2 Avril 1994. Puis, le 10 Avril 1996, NFS a factorisé RSA-130, et a ainsi battu le record établis par QS 2 ans auparavant. Puis, en 1999, SNFS (une version modifiée de NFS) a factorisé un nombre à 211 chiffres : $(10^{211}-1)/{9}$. (selon \cite{ref6} ). Dès lors, NFS a obtenu de nombreux records de factorisation comme RSA-576 en 2003, RSA-640 en 2005 et RSA-768 en 2009.
...
\abstract
L'objectif principal de mon stage était de partir de la version de Laurent Small NFS (idée qu'il a développé dans sa thèse), et d'essayer au lieu d'imposer un spécial-Q d'un seul côté, d'en imposer un de chaque côté.
...
LLL() est une fonction sage qui permets de changer une matrice en une semblable plus agréable pour les algorithmes

arrange\_matrix(M) : transforme une matrice M en une semblable de la forme utilisé par l'algorithme (de vecteur de base (q 0) et (r, 1) avec $f(r) \equiv 0 [q] $ )

\end{nota}

\begin{pseudocode}[shadowbox]{Small NFS avec 2 spécial-Qs}{N, f_1, f_2, B_1, B_2, qrange_1, qrange_2, H_1, H_2, thresh}

q_1 = qrange_1[0]\\
q_2 = qrange_2[0]\\
Res = [ ] \\
\SI q_1 \notin \p  \FAIRE
 q_1 = min (i \in  \p  | i > q_1) \\
 \SI q_2 \notin  \p  \FAIRE
 q_2 = min (i \in  \p  | i > q_2) \\
\TANTQUE q_1 < qrange_1[1] \FAIRE
\BEGIN
   ideal_1 = construit\_ideal(f_1, q_1) \\
   \TANTQUE q_2 < qrange_2[1] \FAIRE
   \BEGIN
...
ideal = intersect\_ideal(ideal_1, ideal_2) \\
M = arrange\_matrix(ideal\_matrix(ideal).LLL()) \\
R = [ ] \\
...
\POUR j \in \{1,2\} \FAIRE
\BEGIN
   \POUR  i \in [2, fbb_j] \cap  \p   \FAIRE
   \BEGIN
   r = racine(M,i) \\
    \SI i[0]<2*H[0] \\       
  \FAIRE
         crible\_lineaire(i[0],r,H_j,array\_norms, j)
   \SINON
          \FAIRE crible\_algebrique( i[0], r, H_j, array\_norms, j)
   \END
\END\\

\COMMENT{Cofactorisation}\\
\POUR i\in [-H[0],H[0]] \FAIRE
\BEGIN
   \POUR j \in [0,H[1]] \FAIRE
   \BEGIN
      \POUR qside \in \{1,2\} \FAIRE
      \BEGIN
         norme = array_norms[ i + H [ 0 ] ] [ j ] \\
         test = norme[0] < thresh[0] \ET norme[1] < thresh[1] \\
         \SI test \FAIRE
         \BEGIN
            (a_0, a_1) = (i_0, i_1) * M \\
            \SI pgcd(a_1,a_1) = 1 \ \ET a_1 >=0  \FAIRE
            \BEGIN
